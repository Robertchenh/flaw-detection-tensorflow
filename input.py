...
def _read_one_batch(path, is_random_label):
    ...
    '''
    data = np.zeros(shape=(200, 64, 64, 3))
    # for i in range(200, 1151):
    for i in range(200, 400):
        #image data
        if i < 10:
            img_name = path + "/000" + str(i) + ".PNG"
        elif 10 <= i <100:
            img_name = path + "/00" + str(i) + ".PNG"
        elif 100 <= i < 1000:
            img_name = path + "/0" + str(i) + ".PNG"
        else:
        ...
        imgrgb01 = imgrgb[:, ::-1, :]
        if i==200:
            imgarray = np.concatenate((imgrgb[np.newaxis, :], imgrgb01[np.newaxis, :]), 0)
        else:
            imgarray = np.concatenate((imgarray, imgrgb[np.newaxis, :]), 0)
            imgarray = np.concatenate((imgarray, imgrgb01[np.newaxis, :]), 0)
        imgrgb02 = imgrgb[::-1, :, :]
        imgarray = np.concatenate((imgarray, imgrgb02[np.newaxis, :]), 0)
        imgrgb03 = imgrgb01[::-1, :, :]
        imgarray = np.concatenate((imgarray, imgrgb03[np.newaxis, :]), 0)
    ...
    labelpath = path + "/Label/Labels.txt"
    f = open(labelpath)
    line = f.readline()
    labellist = []
    while line:
        key = line.split()
        for i in range(4):
            labellist.append(int(key[1]))
        line = f.readline()

    f.close()
    labellist_ = labellist[:800]
    label = np.array(labellist_)
    return data, label



def _read_one_batch_vali(path, is_random_label):
    '''
    The training data contains five data batches in total. The validation data has only one
    batch. This function takes the directory of one batch of data and returns the images and
    corresponding labels as numpy arrays
...
def read_in_all_images_vali(address, shuffle=True, is_random_label = False):
    """
    This function reads all training or validation data, shuffles them if needed, and returns the
    images and the corresponding labels as numpy arrays
    ...
    # for address in address_list:
    print ('Reading images from ' + address)
    data, label = _read_one_batch_vali(address, is_random_label)
    # Concatenate along axis 0 by default
    # data = np.concatenate((data, batch_data))
    # label = np.concatenate((label, batch_label))
...
def prepare_vali_data(padding_size):
    '''
    ...
    data, label = read_in_all_images_vali(vali_dir, is_random_label=TRAIN_RANDOM_LABEL)
...
def prepare_train_data(padding_size):
    ...
    # for i in range(1, NUM_TRAIN_BATCH+1):
    #     path_list.append(full_data_dir + str(i))
    data, label = read_in_all_images(train_dir, is_random_label=TRAIN_RANDOM_LABEL)
...
def get_train_lable(dir):
    labelpath = dir + "/Label/Labels.txt"
    f = open(labelpath)
    line = f.readline()
    labellist = []
    while line:
