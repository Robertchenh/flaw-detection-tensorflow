            ...
            with tf.Session(config=tfconfig) as sess:
                with sess.graph.as_default():
                    ...
                    for step in range(FLAGS.train_steps):
                        ...
                        _, _, train_loss_value, train_error_value, predictions__ = sess.run([self.train_op, self.train_ema_op,
                                                                       self.full_loss, self.train_top1_error, self.predictions_],
                                            {self.image_placeholder: train_batch_data,
                                              self.label_placeholder: train_batch_labels,
                                              self.vali_image_placeholder: validation_batch_data,
                                              self.vali_label_placeholder: validation_batch_labels,
                        ...
                        if step % FLAGS.report_freq == 0:
                            ...
                            for i in range(FLAGS.train_batch_size):
                                if train_batch_labels[i] == 0:
                                    if predictions__[i][0] >= predictions__[i][1]:
                                        ok = ok + 1
                                else:
                                    if predictions__[i][0] <= predictions__[i][1]:
                                        ok = ok + 1
                            error = float(FLAGS.train_batch_size - ok) / float(FLAGS.train_batch_size)
